...
class STLScanner:
    ...
    def parse_stl_metadata(self, file_path: str, root_path: str) -> Optional[Dict]:
        ...
        logger.debug(f"parse: {os.path.basename(file_path)} -> rel='{relative_dir}'")
    ...
    def _add_archive_entries(self, root_path: str, project_id: int, progress_callback=None, cancel_token=None):
        """Добавляет записи о найденных архивах."""
        added = 0
        for dirpath, _, filenames in os.walk(root_path):
